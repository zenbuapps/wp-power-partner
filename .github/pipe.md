...
| 段 | 核心動作 |
|----|---------|
| **A** 前置 | eyes reaction → checkout → `resolve_branch`（精確匹配 `issue/{N}`，fallback 到 `issue/{N}-*`，新建 `issue/{N}`）→ HTTPS → `save_sha` |
...
| **D** 橋接 | `detect_specs`（比對 `specs/` diff，early exit 時同步設定 `specs_available=false`）→ `dynamic_upgrade`（`PR_MODE!=true` + interactive + 生成 specs → 升級 pipeline_mode）→ 通知留言 |
...
| **K** AI 驗收 | `detect_smoke` 檢查 diff 有無動到 `js/src/`、`inc/classes/` → 標記 `lc_bypass_applied`（plugin.php 已內建 `'lc' => false`，無需注入）→ 建置前端（`pnpm build && pnpm build:wp`）→ Playwright 裝 chromium → `run_ai_acceptance`（agent=`zenbu-powers:browser-tester`，admin slug=`power-partner`） |
